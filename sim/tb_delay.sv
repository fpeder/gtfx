...
module tb_delay;
  ...
  initial begin
    ...
    do_reset();
    check("Output is zero after reset", audio_out === 0, $sformatf("Got %0d", audio_out));
    ...
    $display("\nTEST 2: Empty delay line outputs zero (wet-only)");
    do_reset();
    
    cap_reset();
    run_cap(1, 24'sd100000);
    $display("  cap[0] = %0d (expect 0)", cap[0]);
    check("Empty delay line outputs zero", cap[0] == 24'sd0, $sformatf("Got %0d", cap[0]));
    ...
    begin
      ...
      check("Echo appears in expected window", echo_idx >= DELAY && echo_idx <= DELAY + PIPE + 2,
    ...
    begin
      ...
      check("1st echo > 2nd echo (decay)", p1 > p2, $sformatf("%0d > %0d", p1, p2));
    end
    ...
    begin
      ...
      begin
        ...
        check("Bright echo > dark echo", bright_peak > dark_peak, $sformatf(
              "bright=%0d dark=%0d", bright_peak, dark_peak));
      end
    end
    ...
    begin
      ...
      begin
        ...
        check("Full > half", full_pk > half_pk, $sformatf("full=%0d half=%0d", full_pk, half_pk));
        if (full_pk > 0) begin
          check("Half ~ 50% of full", half_pk > (full_pk / 4) && half_pk < (full_pk * 3 / 4),
    ...
    begin
      ...
      begin
        ...
        check("Long echo arrives later", long_idx > short_idx, $sformatf(
              "long=%0d short=%0d", long_idx, short_idx));
      end
    end
    ...
    // =================================================================
    // TEST 10: Zero level -> output = 0 (wet only)
    // =================================================================
    $display("\nTEST 10: Zero level -> output equals 0");
    do_reset();
    level_val    = 8'h00;
    feedback_val = 8'h00;
    tone_val     = 8'hFF;
    ...
    // Now switch input and check output = 0 (wet path muted)
    cap_reset();
    run_cap(5, 24'sd123456);
    $display("  Output samples with level=0:");
    dump_cap(0, 4);

    // audio_out = (lpf_state * 0)>>>8 = 0
    check("Output = 0 when level=0", cap[0] == 24'sd0, $sformatf(
          "Got cap[0]=%0d", cap[0]));
    ...
    check("Clamp to RAM_DEPTH-1", dut.delay_samples == RAM_DEPTH - 1, $sformatf(
          "Got %0d, expected %0d", dut.delay_samples, RAM_DEPTH - 1));

    time_val = 32'd0;
    #1;
    check("Clamp zero to 1", dut.delay_samples == 1, $sformatf("Got %0d", dut.delay_samples));

    time_val = 32'd30;
    #1;
    check("Normal value passthrough", dut.delay_samples == 30, $sformatf(
          "Got %0d", dut.delay_samples));
    ...
    begin
      ...
      check("Echo within 5% of expected", pk > 24'sd370000 && pk < 24'sd410000, $sformatf(
            "Got %0d", pk));
    end
