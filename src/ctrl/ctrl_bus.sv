...
//   0x20..0x27 : Slot 4  - flanger    [0]=man [1]=wid [2]=spd [3]=reg [4]=mix [7]=bypass
//   0x28..0x2F : Slot 5  - reverb     [0]=dec [1]=dmp [2]=mix [3]=pre [4]=ton [5]=lvl [7]=bypass
//   0x30..0x37 : Slot 6  - compressor [0]=thr [1]=rat [2]=atk [3]=rel [4]=mak [7]=bypass
//   0x38..0x3F : Slot 7  - wah        [0]=frq [1]=res [2]=sns [3]=mod [4]=mix [7]=bypass
//   0x40..0x57 : (unused)
//   0x60..0x68 : route[0] .. route[N_XBAR-1]
//
// "set <efx> on"  writes 0x00 to addr 0x07/0x0F/0x17/0x1F/0x27/0x2F/0x37/0x3F (bypass off = active)
...
module ctrl_bus #(
    ...
    // =========================================================================
    localparam int SLOT_END   = N_SLOTS * REGS_PER;   // 0x58 for 11 slots
    localparam int ROUTE_BASE = 8'h60;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            cfg_mem[39] <= 8'h01;   // fln bypass
            cfg_mem[47] <= 8'h01;   // rev bypass
            cfg_mem[55] <= 8'h01;   // cmp bypass
            cfg_mem[63] <= 8'h01;   // wah bypass
            ...
            // ---- Slot 4 (flanger): manual/width/speed/regen/mix ----
            cfg_mem[32] <= 8'h40;   // manual (tighter ~1.6 ms)
            cfg_mem[33] <= 8'h80;   // width  (50% sweep)
            cfg_mem[34] <= 8'h30;   // speed
            cfg_mem[35] <= 8'h90;   // regen  (12.5% feedback)
            cfg_mem[36] <= 8'h60;   // mix    (37.5% wet)
            // cfg_mem[39] = 0x01 bypass on - set above

            // ---- Slot 5 (reverb): decay/damping/mix/pre_dly/tone/level ----
            cfg_mem[40] <= 8'h80;   // decay
            cfg_mem[41] <= 8'h60;   // damping
            cfg_mem[42] <= 8'h60;   // mix (37.5% wet, additive)
            cfg_mem[43] <= 8'h20;   // pre-delay
            cfg_mem[44] <= 8'h80;   // tone
            cfg_mem[45] <= 8'h80;   // level
            // cfg_mem[47] = 0x01 bypass on - set above

            // ---- Slot 6 (compressor): threshold/ratio/attack/release/makeup ----
            cfg_mem[48] <= 8'h60;   // threshold
            cfg_mem[49] <= 8'h40;   // ratio (moderate)
            cfg_mem[50] <= 8'h20;   // attack
            cfg_mem[51] <= 8'h40;   // release
            cfg_mem[52] <= 8'h40;   // makeup gain
            // cfg_mem[55] = 0x01 bypass on - set above

            // ---- Slot 7 (wah): freq/resonance/sensitivity/mode/mix ----
            cfg_mem[56] <= 8'h80;   // freq (centre)
            cfg_mem[57] <= 8'h60;   // resonance
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            // Default chain: ADC→WAH→CMP→PHA→FLN→CHO→TRM→DLY→REV→DAC
            route[0] <= SEL_W'(6);   // DAC ← REV  (port 6)
            route[1] <= SEL_W'(3);   // TRM ← CHO  (port 3)
            route[2] <= SEL_W'(7);   // PHA ← CMP  (port 7)
            route[3] <= SEL_W'(5);   // CHO ← FLN  (port 5)
            route[4] <= SEL_W'(1);   // DLY ← TRM  (port 1)
            route[5] <= SEL_W'(2);   // FLN ← PHA  (port 2)
            route[6] <= SEL_W'(4);   // REV ← DLY  (port 4)
            route[7] <= SEL_W'(8);   // CMP ← WAH  (port 8)
            route[8] <= SEL_W'(0);   // WAH ← ADC  (port 0)
        end else if (wr_en &&
