      ...
      case (state)
        ...
        S_CHECK_CMD: begin
          ...
          if (cmd_buf[0] == "s" && cmd_buf[1] == "t"
            ...
            if (sw_effect == 4'b0000) state <= S_PREP_STATUS_BYP;
                    ...
                    && cmd_buf[7]==" ") begin
            if (cmd_buf[8] == "l") begin
              parse_ptr <= 14;
              target_reg <= 1;
              state <= S_PARSE_INIT;
            end else if (cmd_buf[8] == "f" && cmd_buf[9] == "e") begin
              parse_ptr <= 17;
              target_reg <= 2;
              state <= S_PARSE_INIT;
            end else if (cmd_buf[8] == "t" && cmd_buf[9] == "i") begin
              parse_ptr <= 13;
              target_reg <= 3;
              state <= S_PARSE_INIT;
            end else if (cmd_buf[8] == "t" && cmd_buf[9] == "o") begin
              parse_ptr <= 13;
              target_reg <= 4;
              state <= S_PARSE_INIT;
            end else begin
              ...
              return_state <= S_LOAD_PROMPT;
                    ...
                    && cmd_buf[7]==" ") begin
            if (cmd_buf[8] == "r") begin
              parse_ptr <= 13;
              target_reg <= 5;
              state <= S_PARSE_INIT;
            end else if (cmd_buf[8] == "d") begin
              parse_ptr <= 14;
              target_reg <= 6;
              state <= S_PARSE_INIT;
            end else if (cmd_buf[8] == "e" && cmd_buf[9] == "f") begin
              parse_ptr <= 12;
              target_reg <= 7;
              state <= S_PARSE_INIT;
            end else if (cmd_buf[8] == "e" && cmd_buf[9] == "q" && cmd_buf[10] == "h") begin
              parse_ptr <= 13;
              target_reg <= 8;
              state <= S_PARSE_INIT;
            end else if (cmd_buf[8] == "e" && cmd_buf[9] == "q" && cmd_buf[10] == "l") begin
              parse_ptr <= 13;
              target_reg <= 9;
              state <= S_PARSE_INIT;
            end else begin
              ...
              return_state <= S_LOAD_PROMPT;
                    ...
                    && cmd_buf[7]==" ") begin
            if (cmd_buf[8] == "s") begin
              parse_ptr <= 14;
              target_reg <= 10;
              state <= S_PARSE_INIT;
            end else if (cmd_buf[8] == "f") begin
              parse_ptr <= 13;
              target_reg <= 11;
              state <= S_PARSE_INIT;
            end else begin
        ...
        S_PREP_STATUS_DLY: begin
          if (sw_effect[3]) begin
            ...
            tx_buf[4] <= "L";
            tx_buf[5] <= ":";
            tx_buf[6] <= n2h(reg_level[7:4]);
            tx_buf[7] <= n2h(reg_level[3:0]);
            tx_buf[8] <= " ";
            tx_buf[9] <= "F";
            tx_buf[10] <= ":";
            tx_buf[11] <= n2h(reg_fb[7:4]);
            tx_buf[12] <= n2h(reg_fb[3:0]);
            tx_buf[13] <= " ";
            tx_buf[14] <= "N";
            tx_buf[15] <= ":";
            tx_buf[16] <= n2h(reg_tone[7:4]);
            tx_buf[17] <= n2h(reg_tone[3:0]);
            tx_buf[18] <= " ";
            tx_buf[19] <= "T";
            tx_buf[20] <= ":";
            tx_buf[21] <= n2h(reg_time[31:28]);
            tx_buf[22] <= n2h(reg_time[27:24]);
            tx_buf[23] <= n2h(reg_time[23:20]);
            tx_buf[24] <= n2h(reg_time[19:16]);
            tx_buf[25] <= n2h(reg_time[15:12]);
            tx_buf[26] <= n2h(reg_time[11:8]);
            tx_buf[27] <= n2h(reg_time[7:4]);
            tx_buf[28] <= n2h(reg_time[3:0]);
            tx_buf[29] <= CHAR_CR;
            tx_buf[30] <= CHAR_LF;
            tx_len <= 31;
            tx_idx <= 0;
        ...
        S_PREP_STATUS_CHO: begin
          if (sw_effect[2]) begin
            ...
            tx_buf[4] <= "R";
            tx_buf[5] <= ":";
            tx_buf[6] <= n2h(reg_ch_rate[7:4]);
            tx_buf[7] <= n2h(reg_ch_rate[3:0]);
            tx_buf[8] <= " ";
            tx_buf[9] <= "D";
            tx_buf[10] <= ":";
            tx_buf[11] <= n2h(reg_ch_depth[7:4]);
            tx_buf[12] <= n2h(reg_ch_depth[3:0]);
            tx_buf[13] <= " ";
            tx_buf[14] <= "E";
            tx_buf[15] <= ":";
            tx_buf[16] <= n2h(reg_ch_efx[7:4]);
            tx_buf[17] <= n2h(reg_ch_efx[3:0]);
            tx_buf[18] <= " ";
            tx_buf[19] <= "H";
            tx_buf[20] <= ":";
            tx_buf[21] <= n2h(reg_ch_eqhi[7:4]);
            tx_buf[22] <= n2h(reg_ch_eqhi[3:0]);
            tx_buf[23] <= " ";
            tx_buf[24] <= "L";
            tx_buf[25] <= ":";
            tx_buf[26] <= n2h(reg_ch_eqlo[7:4]);
            tx_buf[27] <= n2h(reg_ch_eqlo[3:0]);
            tx_buf[28] <= CHAR_CR;
            tx_buf[29] <= CHAR_LF;
            tx_len <= 30;
            tx_idx <= 0;
        ...
        S_PREP_STATUS_PHA: begin
          if (sw_effect[1]) begin
            ...
            tx_buf[4] <= "S";
            tx_buf[5] <= ":";
            tx_buf[6] <= n2h(reg_pha_speed[7:4]);
            tx_buf[7] <= n2h(reg_pha_speed[3:0]);
            tx_buf[8] <= " ";
            tx_buf[9] <= "B";
            tx_buf[10] <= ":";
        ...
        S_PREP_STATUS_TRM: begin
          if (sw_effect[0]) begin
            ...
            tx_buf[4] <= "R";
            tx_buf[5] <= ":";
            tx_buf[6] <= n2h(reg_trem_rate[7:4]);
            tx_buf[7] <= n2h(reg_trem_rate[3:0]);
            tx_buf[8] <= " ";
            tx_buf[9] <= "D";
            tx_buf[10] <= ":";
            tx_buf[11] <= n2h(reg_trem_depth[7:4]);
            tx_buf[12] <= n2h(reg_trem_depth[3:0]);
            tx_buf[13] <= " ";
            tx_buf[14] <= "W";
            tx_buf[15] <= ":";
        ...
        S_PARSE_LOOP: begin
          ...
          // ADDED: space skipping to prevent whitespace from aborting parses
          if (c == CHAR_SPC) begin
